...
function main_loop()
    ...
    if (frame % 1 == 0) and (frame > 40) then
        ...
        --joypad.write(1, {A=true, up=false, left=false, B=false, select=false, right=true, down=false, start=false})
        b = b:gsub("%{", "")
        b = b:gsub("%}", "")
        b = b:gsub("%\"", "")
        ...
        --print(frame,b)
        if skrive_screen == true then
            file_matrix = io.open("C:\\Users\\hanso\\Desktop\\MariPlayer\\Prepros_training_data\\screen_matrix_live.csv", "w")
            file_matrix:write(b)
            file_matrix:close()
        end
        --print("ferdig screen matrix")
        ...
        file_step = io.open("C:\\Users\\hanso\\Desktop\\MariPlayer\\Prepros_training_data\\step.txt", "r")
        next_step = file_step:read()
        next_step = tostring(next_step)
        print(frame, next_step)
        action_step(action)

        if (next_step == '1') then
            file_action = io.open("C:\\Users\\hanso\\Desktop\\MariPlayer\\Prepros_training_data\\action.txt", "r")
        ...
        file_step:close()
        -- Utføre action, og ta neste stamp

        file_step = io.open("C:\\Users\\hanso\\Desktop\\MariPlayer\\Prepros_training_data\\step.txt", "w")
        file_step:write(0)
        file_step:close()
    end
...
action = '0'
function action_step(action)

    if action == '0' then
        joypad.write(1, {A=true, up=false, left=false, B=false, select=false, right=false, down=false, start=false})
        print("kom på action 0")
    end
    if action == '1' then
        joypad.write(1, {A=false, up=false, left=false, B=false, select=false, right=true, down=false, start=false})
        print("kom på action 1")

    end
    if action == '2' then
        
        joypad.write(1, {A=true, up=false, left=false, B=false, select=false, right=true, down=false, start=false})
        print("kom på action 2")
